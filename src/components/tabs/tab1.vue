...
export default {
  ...
  mounted() {
    ...
    if(hjxdata){
        this.color = hjxdata.color;
        this.brightness = hjxdata.brightness;
        this.blur = hjxdata.blur;
    }else{
  ...
  methods: {
    ...
    submitdata() {      
        ...
        setTimeout(() => {
            this.loading = false;
            // 注意数字String格式化
            this.setCookie('hjxdata', {color:this.color,brightness:String(this.brightness),blur:String(this.blur),backgroundblur:String(this.backgroundblur)},0.005);
            location.reload();
        }, 800)   
    },
    redefault(){              
